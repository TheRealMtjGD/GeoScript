...
def main() -> None:
    ...
    if config_env['buildmode'] == 'std':
        ...
        if compile_requirements['compile-mode'] == 'Standard':
            ...
            tokenised = gsparser.GSParser(gspackage, config_env['file'], {'hm': heap_memory, 'ids': identifier_stack}).return_parser()
        ...
        output = config_env['file'].replace('.gss', '.gsc')
        
        logging.logToFile(logging.P, 'Starting compilation (mode: gsc)', logging.CompileLog())
        ...
        logging.logToFile(logging.P, 'Dumping compile data into .gsc', logging.CompileLog())
        dumpGSCRequirements(str(abstract_syntax_tree), {}, heap_memory, identifier_stack, main_env['heap_memory_max'], output)
